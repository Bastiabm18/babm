...
const userMenuItems: MenuItem[] = [
  ...
  { key: 'promos', path: '/dashboard/promos', icon: <FaChartBar />, role: ['user', 'admin'] },
  { key: 'faq', path: '/dashboard/faq', icon: <FaQuestion />, role: ['user', 'admin'] },
  { key: 'settings', path: '/dashboard/config', icon: <AiFillSetting/>, role: ['user', 'admin'] },
  {key: 'quit', path: '/', icon: <BsBackspace />, role: ['user', 'admin']}, // Ejemplo de item para salir
];
...
export default function DashboardContent({ userName, lang }: DashboardContentProps) {
  ...
  return (
    <div className="bg-primary-light dark:bg-background-dark_alt shadow rounded-lg p-6">
      <h1 className="text-2xl font-bold text-gray-900 dark:text-primary-dark mb-6">
        Bienvenido a tu Panel, {userName || 'Usuario'}!
      </h1>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {userMenuItems.map((item) => (
          <Link key={item.key} href={`/${lang}${item.path}`} className="group">
            <div className="flex flex-col items-center justify-center p-6 rounded-lg bg-background-light_alt dark:bg-background-dark text-gray-800 dark:text-gray-200 cursor-pointer transform transition duration-200 hover:scale-105 hover:bg-primary-light/20 dark:hover:bg-primary-dark/30">
              <div className="text-3xl mb-3 text-primary-light_alt dark:text-primary-dark">{item.icon}</div>
